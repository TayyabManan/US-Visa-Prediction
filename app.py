...
from visa_approval_prediction.constants import APP_HOST, APP_PORT
from visa_approval_prediction.pipeline.prediction_pipeline import visaData, visaClassifier
...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
            request, name="visa.html", context={"context": "Rendering"})
...
async def predict_api(data: PredictRequest):
    try:
        import traceback
        print(">>> Step 1: Building input data", flush=True)
        visa_data = visaData(
        ...
        print(f">>> Step 2: DataFrame built, shape={visa_df.shape}, columns={list(visa_df.columns)}", flush=True)

        model_predictor = visaClassifier()
        print(">>> Step 3: Loading model...", flush=True)
        prediction, confidence = model_predictor.predict_with_confidence(dataframe=visa_df)
        print(f">>> Step 4: Prediction done: {prediction}, confidence={confidence}", flush=True)

        # TargetValueMapping: Certified=0, Denied=1
        result = "approved" if prediction == 0 else "denied"
        input_data = data.dict()
        insights = model_predictor.explain(visa_df, input_data, result)
